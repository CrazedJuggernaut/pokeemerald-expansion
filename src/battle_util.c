...
void HandleAction_UseMove(void)
{
        ...
        && GetBattlerSide(gBattlerAttacker) != GetBattlerSide(gSideTimers[side].followmeTarget))
    {
        gBattleStruct->moveTarget[gBattlerAttacker] = gBattlerTarget = gSideTimers[side].followmeTarget; // follow me moxie fix
           ...
           && ((GetBattlerAbility(*(gBattleStruct->moveTarget + gBattlerAttacker)) != ABILITY_LIGHTNING_ROD && moveType == TYPE_ELECTRIC)
            || (GetBattlerAbility(*(gBattleStruct->moveTarget + gBattlerAttacker)) != ABILITY_PINACEAE && moveType == TYPE_ELECTRIC)
            || (GetBattlerAbility(*(gBattleStruct->moveTarget + gBattlerAttacker)) != ABILITY_STORM_DRAIN && moveType == TYPE_WATER)))
    {
        ...
        for (gActiveBattler = 0; gActiveBattler < gBattlersCount; gActiveBattler++)
        {
            if (side != GetBattlerSide(gActiveBattler)
                ...
                && ((GetBattlerAbility(gActiveBattler) == ABILITY_LIGHTNING_ROD && moveType == TYPE_ELECTRIC)
                 || (GetBattlerAbility(gActiveBattler) == ABILITY_PINACEAE && moveType == TYPE_ELECTRIC)
        ...
        else
        {
            ...
            || (battlerAbility == ABILITY_PINACEAE && gCurrentMove != MOVE_TEATIME))
...
static const u8 sAbilitiesAffectedByMoldBreaker[] =
{
    ...
    [ABILITY_APPARITION] = 1,
    [ABILITY_CIRCULATE] = 1,
    [ABILITY_UNBREAKABLE] = 1,
    [ABILITY_BIRD_EATER] = 1,
    [ABILITY_GALLANT] = 1,
    [ABILITY_PINACEAE] = 1,
    [ABILITY_NIMBLE] = 1,
    [ABILITY_KAKTOS] = 1,
    ...
    [ABILITY_WILLPOWER] = 1,
    [ABILITY_IRON_CLAD] = 1,
    [ABILITY_LUNAR_GUARDIAN] = 1,
    [ABILITY_FROZEN_HUSK] = 1,
};
...
void PrepareStringBattle(u16 stringId, u8 battler)
{
    ...
    // Check Defiant and Competitive stat raise whenever a stat is lowered.
              ...
              && ((targetAbility == ABILITY_DEFIANT && CompareStat(gBattlerTarget, STAT_ATK, MAX_STAT_STAGE, CMP_LESS_THAN))
                 || (targetAbility == ABILITY_HYPER_CUTTER && CompareStat(gBattlerTarget, STAT_ATK, MAX_STAT_STAGE, CMP_LESS_THAN))
              ...
              && !(gBattleScripting.stickyWebStatDrop == 1 && gSideTimers[targetSide].stickyWebBattlerSide == targetSide)) // Sticky Web must have been set by the foe
    {
        ...
        else if (GetBattlerAbility(gBattlerTarget) == ABILITY_HYPER_CUTTER)
        {
			gBattleScripting.abilityPopupOverwrite = ABILITY_HYPER_CUTTER;
			gLastUsedAbility = ABILITY_HYPER_CUTTER;
            SET_STATCHANGER(STAT_ATK, 2, FALSE);
        }
        else if (GetBattlerAbility(gBattlerTarget) == ABILITY_COMPETITIVE)
        {
			gBattleScripting.abilityPopupOverwrite = ABILITY_COMPETITIVE;
			gLastUsedAbility = ABILITY_DEFIANT;
            SET_STATCHANGER(STAT_SPATK, 2, FALSE);
        }
        else if (GetBattlerAbility(gBattlerTarget) == ABILITY_RUN_AWAY)
        {
...
u8 DoBattlerEndTurnEffects(void)
{
    ...
    while (gBattleStruct->turnEffectsBattlerId < gBattlersCount && gBattleStruct->turnEffectsTracker <= ENDTURN_BATTLER_COUNT)
    {
        ...
        case ENDTURN_YAWN:  // yawn
            if (gStatuses3[gActiveBattler] & STATUS3_YAWN)
            {
                ...
                if (!(gStatuses3[gActiveBattler] & STATUS3_YAWN) && !(gBattleMons[gActiveBattler].status1 & STATUS1_ANY)
                 ...
                 && IsAbilityOnSide(battlerAbility, ABILITY_LUNAR_GUARDIAN)
        ...
        case ENDTURN_SLOW_START:
            ...
            gBattleStruct->turnEffectsTracker++;
            break;
        case ENDTURN_EARLY_BIRD:
            if (gDisableStructs[gActiveBattler].slowStartTimer
                && --gDisableStructs[gActiveBattler].slowStartTimer == 0
                && ability == ABILITY_EARLY_BIRD)
            {
                BattleScriptExecute(BattleScript_EarlyBirdEnds);
                effect++;
            }
            gBattleStruct->turnEffectsTracker++;
            break;
    ...
    do
    {
        ...
        case CANCELLER_ASLEEP: // check being asleep
            if (gBattleMons[gBattlerAttacker].status1 & STATUS1_SLEEP)
            {
                ...
                if (GetBattlerAbility(gBattlerAttacker) == ABILITY_LUNAR_GUARDIAN)
                {
                    gBattleMons[gBattlerAttacker].status1 &= ~STATUS1_SLEEP;
                    gBattleMons[gBattlerAttacker].status2 &= ~STATUS2_NIGHTMARE;
                    BattleScriptPushCursor();
                    gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_WOKE_UP_LUNAR_GUARDIAN;
                    gBattlescriptCurrInstr = BattleScript_MoveUsedWokeUp;
                    effect = 2;
                }
                if (IsAbilityOnSide(gBattlerAttacker, ABILITY_LUNAR_GUARDIAN))
                {
                    gBattleMons[gBattlerAttacker].status1 &= ~STATUS1_SLEEP;
                    gBattleMons[gBattlerAttacker].status2 &= ~STATUS2_NIGHTMARE;
                    BattleScriptPushCursor();
        ...
        case CANCELLER_POWDER_MOVE:
            if ((gBattleMoves[gCurrentMove].flags & FLAG_POWDER) && (gBattlerAttacker != gBattlerTarget))
            {
                ...
                if (IS_BATTLER_OF_TYPE(gBattlerTarget, TYPE_GRASS) || GetBattlerAbility(gBattlerTarget) == ABILITY_OVERCOAT || GetBattlerAbility(gBattlerTarget) == ABILITY_WATER_VEIL)
            #else
    ...
    case ABILITYEFFECT_ON_SWITCHIN: // 0
        ...
        case ABILITY_EARLY_BIRD:
            if (!gSpecialStatuses[battler].switchInAbilityDone)
            {
                ...
                gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_SWITCHIN_EARLYBIRD;
                gSpecialStatuses[battler].switchInAbilityDone = TRUE;
                BattleScriptPushCursorAndCallback(BattleScript_SwitchInAbilityMsg);
                effect++;
            }
            break;
        case ABILITY_SPACE_DISTORTION:
            if (!gSpecialStatuses[battler].switchInAbilityDone &&
			    !(gFieldStatuses & STATUS_FIELD_GRAVITY))
            {
                gSpecialStatuses[battler].switchInAbilityDone = TRUE;
				gBattleScripting.abilityPopupOverwrite = gLastUsedAbility = ABILITY_SPACE_DISTORTION;
				gFieldStatuses |= STATUS_FIELD_GRAVITY;
				gFieldTimers.gravityTimer = 3;
				BattleScriptPushCursorAndCallback(BattleScript_SpaceDistortionActivated);
				effect++;
			}
			break;
        ...
        case ABILITY_WILLPOWER:
            if (!gSpecialStatuses[battler].switchInAbilityDone)
            {
                gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_SWITCHIN_WILLPOWER;
                gSpecialStatuses[battler].switchInAbilityDone = TRUE;
                BattleScriptPushCursorAndCallback(BattleScript_SwitchInAbilityMsg);
                effect++;
            }
        ...
        case ABILITY_LUNAR_GUARDIAN:
            ...
            {
                gBattlerTarget = battler;
                gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_SWITCHIN_LUNAR_GUARDIAN;
                BattleScriptPushCursorAndCallback(BattleScript_LunarGuardianActivates);
                effect++;
                gSpecialStatuses[battler].switchInAbilityDone = TRUE;
            }
            break;
        ...
        case ABILITY_INTIMIDATE:
            if (!gSpecialStatuses[battler].switchInAbilityDone)
            {
                ...
                BattleScriptPushCursorAndCallback(BattleScript_TimeManipulatorActivates);
                effect++;
            }
            break;
        case ABILITY_TIME_MANIPULATOR:
            if (!gSpecialStatuses[battler].switchInAbilityDone)
            {
                gBattlerAttacker = battler;
                gSpecialStatuses[battler].switchInAbilityDone = TRUE;
            ...
            case ABILITY_ROYAL_HONEY:
                {
                    ...
                    gBattleMoveDamage = gBattleMons[gActiveBattler].maxHP / 8;
                        if (gBattleMoveDamage == 0)
                            gBattleMoveDamage = 1;
                        gBattleMoveDamage *= -1;
                        BattleScriptExecute(BattleScript_RoyalHoneyActivates);
                        effect++;
                    }
                }
                break;
            case ABILITY_ROCK_BODY:
                {
                    if (!BATTLER_MAX_HP(gActiveBattler) && !(gStatuses3[gActiveBattler] & STATUS3_HEAL_BLOCK))
                {
                    gBattleMoveDamage = gBattleMons[gActiveBattler].maxHP / 8;
    ...
    case ABILITYEFFECT_MOVES_BLOCK: // 2
        {
            ...
            if ((gLastUsedAbility == ABILITY_SOUNDPROOF && gBattleMoves[move].flags & FLAG_SOUND && !(moveTarget & MOVE_TARGET_USER))
             ...
             || (gLastUsedAbility == ABILITY_IRON_CLAD && gBattleMoves[move].flags & FLAG_SLICING_MOVE))
            {
            ...
            else if (BlocksPrankster(move, gBattlerAttacker, gBattlerTarget, TRUE) && !(IS_MOVE_STATUS(move) && targetAbility == ABILITY_KNOWLEDGE))
            {
                if (!(gBattleTypeFlags & BATTLE_TYPE_DOUBLE) || !(moveTarget & (MOVE_TARGET_BOTH | MOVE_TARGET_FOES_AND_ALLY)))
                    CancelMultiTurnMoves(gBattlerAttacker); // Don't cancel moves that can hit two targets bc one target might not be protected
                gBattleScripting.battler = gBattlerAbility = gBattlerTarget;
        ...
        case ABILITY_BERSERK:
         ...
         case ABILITY_UNBREAKABLE:
               ...
               ||CompareStat(battler, STAT_SPDEF, MAX_STAT_STAGE, CMP_LESS_THAN)))
            {
                ...
                BattleScriptPushCursor();
                gBattlescriptCurrInstr = BattleScript_UnbreakableActivates;
                effect++;
            }
            break;
...
u32 GetBattlerAbility(u8 battlerId)
{
    ...
    if ((((gBattleMons[gBattlerAttacker].ability == ABILITY_MOLD_BREAKER
            ...
            || gBattleMons[gBattlerAttacker].ability == ABILITY_DISTORTED_LOGIC)
...
bool32 CanSleep(u8 battlerId)
{
    ...
    if (ability == ABILITY_INSOMNIA
      ...
      || IsAbilityOnSide(battlerId, ABILITY_LUNAR_GUARDIAN)
...
bool32 CanBeConfused(u8 battlerId)
{
    if (GetBattlerAbility(battlerId) == ABILITY_OWN_TEMPO
      || GetBattlerAbility(battlerId) == ABILITY_UNBREAKABLE
...
u32 GetMoveTarget(u16 move, u8 setTarget)
{
    ...
    case MOVE_TARGET_SELECTED:
        ...
        else
        {
            ...
            else if (gBattleMoves[move].type == TYPE_ELECTRIC
                && IsAbilityOnOpposingSide(gBattlerAttacker, ABILITY_PINACEAE)
                && GetBattlerAbility(targetBattler) != ABILITY_PINACEAE)
            {
                targetBattler ^= BIT_FLANK;
                RecordAbilityBattle(targetBattler, gBattleMons[targetBattler].ability);
                gSpecialStatuses[targetBattler].lightningRodRedirected = TRUE;
...
static bool32 IsBattlerGrounded2(u8 battlerId, bool32 considerInverse)
{
    ...
    if (GetBattlerAbility(battlerId) == ABILITY_LEVITATE)
        return FALSE;
    if (GetBattlerAbility(battlerId) == ABILITY_KNOWLEDGE)
        return FALSE;
...
u32 GetBattlerWeight(u8 battlerId)
{
    ...
    if (ability == ABILITY_HEAVY_METAL)
        ...
        weight /= 2;
    else if (ability == ABILITY_ALLOY_BODY)
        weight /= 2;
...
static u32 CalcMoveBasePowerAfterModifiers(u16 move, u8 battlerAtk, u8 battlerDef, u8 moveType, bool32 updateFlags)
{
	...
	for (i = 0; i < gBattlersCount; i++)
    {
        if (gBattleMons[i].status1 & STATUS1_SLEEP || (GetBattlerAbility(i) == ABILITY_COMATOSE && IsBattlerAlive(i)))
            numsleepmons++;
    }
    ...
    case ABILITY_ANALYTIC:
        if (GetBattlerTurnOrderNum(battlerAtk) == gBattlersCount - 1 && move != MOVE_FUTURE_SIGHT && move != MOVE_DOOM_DESIRE && move != MOVE_RAZOR_WIND)
           MulModifier(&modifier, UQ_4_12(1.3));
        break;
    case ABILITY_NINJITSU:
        if (GetBattlerTurnOrderNum(battlerAtk) == gBattlersCount - 0)
           MulModifier(&modifier, UQ_4_12(1.3));
        break;
    ...
    case ABILITY_SAGE:
        if (moveType == TYPE_PSYCHIC)
           MulModifier(&modifier, UQ_4_12(1.5));
        break;
    case ABILITY_CONTAMINATE:
        if (moveType == TYPE_POISON && gBattleStruct->ateBoost[battlerAtk])
            MulModifier(&modifier, UQ_4_12(1.2));
        break;
    case ABILITY_THE_CREATOR:
        if (moveType == TYPE_NORMAL)
            MulModifier(&modifier, UQ_4_12(1.3));
        break;
    case ABILITY_BAD_DREAMS:
		if (numsleepmons == 1)
            MulModifier(&modifier, UQ_4_12(1.3)); // buffed bc of Sleep Clause
    ...
    case ABILITY_GALLANT:
    case ABILITY_PINACEAE:
        if (gBattleMoves[move].flags & FLAG_SLICING_MOVE)
           MulModifier(&modifier, UQ_4_12(1.5));
        break;
    case ABILITY_STRIKER:
        if (gBattleMoves[move].flags & FLAG_KICKING_MOVE)
           MulModifier(&modifier, UQ_4_12(1.5));
        break;
    case ABILITY_BALLISTICS:
...
static u32 CalcAttackStat(u16 move, u8 battlerAtk, u8 battlerDef, u8 moveType, bool32 isCrit, bool32 updateFlags)
{
    ...
    case ABILITY_LEAF_GUARD:
        if (IS_MOVE_SPECIAL(move) && IsBattlerWeatherAffected(battlerDef, B_WEATHER_SUN))
            MulModifier(&modifier, UQ_4_12(0.5));
        break;
    }
    ...
    case HOLD_EFFECT_LIGHT_BALL:
        if (atkBaseSpeciesId == SPECIES_PIKACHU)
            MulModifier(&modifier, UQ_4_12(2.0));
        if (atkBaseSpeciesId == SPECIES_RAICHU, SPECIES_RAICHU_ALOLAN)
            MulModifier(&modifier, UQ_4_12(1.5));
        break;
...
static u32 CalcFinalDmg(u32 dmg, u16 move, u8 battlerAtk, u8 battlerDef, u8 moveType, u16 typeEffectivenessModifier, bool32 isCrit, bool32 updateFlags)
{
    ...
    {
        if (abilityAtk == ABILITY_AGGRO_SWIM)
    ...
    case ABILITY_NEUROFORCE:
        ...
        break;
    case ABILITY_FERVOR:
        percentBoost = min((gBattleStruct->sameMoveTurns[battlerAtk] * GetBattlerAbility(battlerAtk)), 100);
    ...
    case ABILITY_ROCK_BODY:
    case ABILITY_FROZEN_HUSK:
        if (typeEffectivenessModifier >= UQ_4_12(2.0))
            MulModifier(&finalModifier, UQ_4_12(0.75));
        break;
    case ABILITY_FORMATION:
            MulModifier(&finalModifier, UQ_4_12(0.75));
        break;
    }
...
static void MulByTypeEffectiveness(u16 *modifier, u16 move, u8 moveType, u8 battlerDef, u8 defType, u8 battlerAtk, bool32 recordAbilities)
{
    ...
    if (moveType == TYPE_GRASS && defType == TYPE_BUG && GetBattlerAbility(battlerAtk) == ABILITY_FLY_TRAP && mod == UQ_4_12(0.5))
    {
        mod = UQ_4_12(2.0);
        if (recordAbilities)
            RecordAbilityBattle(battlerAtk, ABILITY_FLY_TRAP);
    }
    if (moveType == TYPE_GRASS && defType == TYPE_FLYING && GetBattlerAbility(battlerAtk) == ABILITY_FLY_TRAP && mod == UQ_4_12(0.5))
    {
        mod = UQ_4_12(2.0);
        if (recordAbilities)
            RecordAbilityBattle(battlerAtk, ABILITY_FLY_TRAP);
    }
    if (moveType == TYPE_GRASS && defType == TYPE_POISON && GetBattlerAbility(battlerAtk) == ABILITY_CLEANSE && mod == UQ_4_12(0.5))
    {
        mod = UQ_4_12(2.0);
        if (recordAbilities)
            RecordAbilityBattle(battlerAtk, ABILITY_CLEANSE);
    }
    if (moveType == TYPE_FIRE && defType == TYPE_ROCK && GetBattlerAbility(battlerAtk) == ABILITY_MOLTEN_BODY && mod == UQ_4_12(0.5))
    {
        mod = UQ_4_12(2.0);
        if (recordAbilities)
            RecordAbilityBattle(battlerAtk, ABILITY_MOLTEN_BODY);
    }
    ...
    else if (gBattleMoves[move].power && GetBattlerAbility(battlerAtk) == ABILITY_DISTORTED_LOGIC && mod == UQ_4_12(0.0))
    {
        mod = UQ_4_12(1.0);
        if (recordAbilities)
            RecordAbilityBattle(battlerAtk, ABILITY_DISTORTED_LOGIC);
    }
...
static u16 CalcTypeEffectivenessMultiplierInternal(u16 move, u8 moveType, u8 battlerAtk, u8 battlerDef, bool32 recordAbilities, u16 modifier)
{
    ...
    {
        modifier = UQ_4_12(0.0);
        if (recordAbilities && defAbility == ABILITY_KNOWLEDGE)
        {
            gLastUsedAbility = ABILITY_KNOWLEDGE;
            gMoveResultFlags |= (MOVE_RESULT_MISSED | MOVE_RESULT_DOESNT_AFFECT_FOE);
            gLastLandedMoves[battlerDef] = 0;
            gBattleCommunication[MISS_TYPE] = B_MSG_GROUND_MISS;
            RecordAbilityBattle(battlerDef, ABILITY_KNOWLEDGE);
        }
    }
    ...
    if (!IsBattlerGrounded(battlerDef) && (GetBattlerAbility(battlerAtk) == ABILITY_DISTORTED_LOGIC)
        && (gBattleMons[battlerDef].type1 == TYPE_FLYING || gBattleMons[battlerDef].type2 == TYPE_FLYING || gBattleMons[battlerDef].type3 == TYPE_FLYING))
    {
        modifier = UQ_4_12(1.0);
    }
    ...
    if (((defAbility == ABILITY_WONDER_GUARD && modifier <= UQ_4_12(1.0))
        ...
        || (defAbility == ABILITY_TIME_MANIPULATOR && battlerDef == BATTLE_PARTNER(battlerAtk)))
...
u16 CalcPartyMonTypeEffectivenessMultiplier(u16 move, u16 speciesDef, u16 abilityDef)
{
    ...
    if (move != MOVE_STRUGGLE && moveType != TYPE_MYSTERY)
    {
        ...
        if (moveType == TYPE_GROUND && abilityDef == ABILITY_LEVITATE && !(gFieldStatuses & STATUS_FIELD_GRAVITY))
            modifier = UQ_4_12(0.0);
        if (moveType == TYPE_GROUND && abilityDef == ABILITY_KNOWLEDGE && !(gFieldStatuses & STATUS_FIELD_GRAVITY))
            modifier = UQ_4_12(0.0);
...
static bool32 IsUnnerveAbilityOnOpposingSide(u8 battlerId)
{
    if (IsAbilityOnOpposingSide(battlerId, ABILITY_UNNERVE)
      ...
      || IsAbilityOnOpposingSide(battlerId, ABILITY_WILLPOWER))
        return TRUE;
    return FALSE;
}
...
static bool8 CanBeInfinitelyConfused(u8 battlerId)
{
    if  (gBattleMons[battlerId].ability == ABILITY_OWN_TEMPO
         || gBattleMons[battlerId].ability == ABILITY_UNBREAKABLE
         || IsBattlerTerrainAffected(battlerId, STATUS_FIELD_MISTY_TERRAIN)
         || gSideStatuses[GetBattlerSide(battlerId)] & SIDE_STATUS_SAFEGUARD)
    {
        return FALSE;
